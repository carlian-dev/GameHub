<script lang="ts">
	...
	function triggerGoogleLink(id: string) {
		...
		// Prompt One Tap / account chooser; callback will handle linking
		g.accounts.id.prompt((notification: any) => {
			if (notification?.isNotDisplayed?.() || notification?.isSkippedMoment?.()) {
				// Fallback: try to render a temporary button if prompt not displayed
				// Create a hidden prompt via renderButton in a popup is complex; just inform admin
...
<style>
	...
	.mono { font-weight: 600; letter-spacing: -0.01em; font-variant-numeric: tabular-nums; }
